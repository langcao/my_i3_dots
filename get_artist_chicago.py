...
def get_query():
	...
	if argc>1:
		for i in range(1, argc):
			key += ' ' + argvs[i]
		url = "https://www.artic.edu/collection?artist_ids=%s"%key
		print(url)
		session = HTMLSession()
		...
		url = "https://www.artic.edu/collection?artist_ids=%s&page=%d"%(key, randint(1, max_page))
	else:
...
if exists('%s.art.lck'%path):
	...
	session = HTMLSession()
	url, key = get_query()
	system('dunstify -u critical " ART INSTITVTE CHICAGO" " Searching artist<i>%s</i>..."'%key)
	print(url)
	response = session.get(url)
	items = []
	...
	if items:
		...
		description =  res[res.find(',')+1:len(res)]
		...
		system('dunstify -u low " ART INSTITVTE CHICAGO" " Searching artist<i>%s </i>Done!"'%key)

...
try:
	f = open('/tmp/cmus_lyrics_id')
	tag = f.read()
	f.close()
	system('dunstify -r %d -u critical " Lyrics file is loading..."'%tag)
except:
	subprocess = subprocess.Popen("dunstify -p -u critical ' Lyrics file is loading...'", shell=True, stdout=subprocess.PIPE)
	out = subprocess.stdout.read()
	tag = int(out)
	system('echo %d > /tmp/cmus_lyrics_id'%tag)
...
try:
	...
	for line in lines:
		if line.find('[')==0:
			...
			times.append(int(line[1:3])*360+int(line[4:6])*60+int(line[7:9]))
